...
describe('detect', () => {
  it('suggests jwt first for a three-segment token', () => {
    ...
    expect(candidates[0]?.id).toBe('jwt');
  });

  it('suggests url for percent-encoded text', () => {
    expect(detect('name=a%20b').some((c) => c.id === 'url')).toBe(true);
  });

  it('suggests unicode for escape sequences', () => {
    expect(detect('\\u0068\\u0069').some((c) => c.id === 'unicode')).toBe(true);
  });

  it('suggests hex for even-length hex strings', () => {
    expect(detect('68656c6c6f').some((c) => c.id === 'hex')).toBe(true);
  });

  it('suggests base64 for multiple-of-4 alphabet strings', () => {
    expect(detect('aGVsbG8gd29ybGQh').some((c) => c.id === 'base64')).toBe(true);
  });
